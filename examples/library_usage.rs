...
fn main() -> Result<()> {
    ...
    if let Err(e) = quick_slideshow("test_images", "example_quick.mp4") {
        warn!(
            "   Error: {} (This is expected if test_images doesn't exist)",
            e
        );
    } else {
    ...
    info!(
        "   Manually added {} images and {} videos",
        generator.image_count(),
        generator.video_count()
    );
    ...
    info!(
        "   Default options: {}s per slide, {:?} resolution",
        generator.options().duration_per_slide,
        generator.options().output_dimensions.unwrap_or((0, 0))
    ); // Handle None case
    ...
    info!(
        "   Updated options: {}s per slide, {:?} resolution",
        generator.options().duration_per_slide,
        generator.options().output_dimensions.unwrap_or((0, 0)),
    );

    // Example 5: Optional dimensions
    info!("5. Optional dimensions:");
    let auto_options = SlideshowOptions::new().with_duration_per_slide(3.0);
    // No output_dimensions specified - will use first image's dimensions
    ...
    info!(
        "Note: Some operations may fail if test files don't exist, which is normal for this demo."
    );
